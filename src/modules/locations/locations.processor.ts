  ...
  async process(job: Job): Promise<void> {
    ...
    try {
      await this.dataSource.transaction(async (manager) => {
        ...
        const areas = await this.areasService.findContainingAreas(
          latitude,
          longitude,
          manager,
        );
        ...
        const ttl =
          this.configService.get<number>('app.entryTtlSeconds') ?? 300;
        ...
        if (logMode === 'entry_only') {
          ...
          for (const area of areas) {
            ...
            await this.logsService.logEntry(
              userId,
              area.id,
              timestamp,
              manager,
      ...
      this.logger.error(
        `Failed to process location job ${job.id} for user ${userId}`,
